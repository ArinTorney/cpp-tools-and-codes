...
7. Platform: Any platform is okay, but usually those which have a greater community are recommended, like Windows or Ubuntu. (MacOS is also okay, but mind the fact it only supports a max version of 4.1, since apple deprecated OpenGL somewhere in 2017.)  
8. Graphics card: Any graphics card which supports a good version of modern OpenGL (which most modern ones do)
