...
3. Huge community: C++ has a huge community and therefore has many libraries to help programming in it, common ones of them licensed under less restrictive (either permissive or weak copyleft) licenses may be Boost for general purpose utility, SFML and Allegro for 2D graphics and multimedia, Godot, Irrlicht, Ogre, Lumberyard for 3D graphics, wxWidgets, FLTK, GLFW, FreeGLUT etc. for GUI (game-centric for GLFW, OpenGL-centric for FreeGLUT, Any GUI for others), OpenAL soft for Audio files, Bullet for physics, GLM for mathematics, Magick++, STB etc. for image loading, OpenCL, OpenGL, OpenCV, Vulkan, CUDA(NVIDIA-specific) etc. for hardware acceleration etc..  
