...
* Text editor: (Notepad++(Windows-only), Sublime Text)(very fast and featured (at least with plugins), but mind the license), BlueFish(seems to have good features, mind the IDE-like UI and lag), (Vim, NeoVim)(awesome if you know the commands, but mind the Unix-style) or (VSCode, Atom)(outstanding at everything (at least with plugins), but mind the lag) is good. IDE should not be required for now (they will in fact be a burden and will be laggy), but still if anybody wants, there are options available like Code::Blocks, CodeLite, Eclipse, Qt Creator, Visual Studio(Windows-only, proprietary), Xcode(MacOS-only, proprietary), CLion(paid proprietary) etc.  
